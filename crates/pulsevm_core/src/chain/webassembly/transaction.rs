...
pub fn send_inline(mut env: FunctionEnvMut<WasmContext>, ptr: WasmPtr<u8>, length: u32) -> Result<(), RuntimeError> {
    {
    ...
    let action = Action::read(&src_bytes, &mut 0).map_err(|e| RuntimeError::new(format!("failed to deserialize inline action: {}", e)))?;
...
pub fn check_transaction_authorization(
    ...
    trx_ptr.slice(&view, trx_length)?.read_slice(&mut trx_bytes)?;
    let transaction = Transaction::read(&trx_bytes, &mut 0).map_err(|e| RuntimeError::new(format!("failed to deserialize transaction: {}", e)))?;
    ...
    if pubkeys_length > 0 {
        ...
        pubkeys_ptr.slice(&view, pubkeys_length)?.read_slice(&mut pubkeys_bytes)?;
        provided_keys = HashSet::<PublicKey>::read(&pubkeys_bytes, &mut 0)
            .map_err(|e| RuntimeError::new(format!("failed to deserialize provided public keys: {}", e)))?;
    }

    if perms_length > 0 {
        let mut perms_bytes = vec![0u8; perms_length as usize];
        perms_ptr.slice(&view, perms_length)?.read_slice(&mut perms_bytes)?;
        provided_permissions = HashSet::<PermissionLevel>::read(&perms_bytes, &mut 0)
            .map_err(|e| RuntimeError::new(format!("failed to deserialize provided permission levels: {}", e)))?;
    }
