...
pub fn db_find_i64(mut env: FunctionEnvMut<WasmContext>, code: u64, scope: u64, table: u64, id: u64) -> Result<i32, RuntimeError> {
...
pub fn db_get_i64(mut env: FunctionEnvMut<WasmContext>, itr: i32, buffer_ptr: WasmPtr<u8>, buffer_len: u32) -> Result<i32, RuntimeError> {
    let (env_data, store) = env.data_and_store_mut();
    let memory = env_data.memory().as_ref().expect("Wasm memory not initialized");
    let view = memory.view(&store);
    let slice = buffer_ptr.slice(&view, buffer_len)?;
...
pub fn db_next_i64(mut env: FunctionEnvMut<WasmContext>, itr: i32, primary_ptr: WasmPtr<u8>) -> Result<i32, RuntimeError> {
    let context = env.data_mut().apply_context_mut();
...
pub fn db_previous_i64(mut env: FunctionEnvMut<WasmContext>, itr: i32, primary_ptr: WasmPtr<u8>) -> Result<i32, RuntimeError> {
    let context = env.data_mut().apply_context_mut();
    let mut next_primary = 0u64;
...
pub fn db_lowerbound_i64(mut env: FunctionEnvMut<WasmContext>, code: u64, scope: u64, table: u64, primary: u64) -> Result<i32, RuntimeError> {
    let context = env.data_mut().apply_context_mut();
    let res = context.db_lowerbound_i64(code.into(), scope.into(), table.into(), primary)?;
    Ok(res)
}

pub fn db_upperbound_i64(mut env: FunctionEnvMut<WasmContext>, code: u64, scope: u64, table: u64, primary: u64) -> Result<i32, RuntimeError> {
    let context = env.data_mut().apply_context_mut();
    let res = context.db_upperbound_i64(code.into(), scope.into(), table.into(), primary)?;
    Ok(res)
}

pub fn db_end_i64(mut env: FunctionEnvMut<WasmContext>, code: u64, scope: u64, table: u64) -> Result<i32, RuntimeError> {
    let context = env.data_mut().apply_context_mut();
