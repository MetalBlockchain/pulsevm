...
use crate::{cli::{Cli, Commands, create, get, set, wallet}, config::load_or_create_config};
...
pub async fn execute(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Set { subcmd } => set::handle(&pulsevm_api_client, &mut config, &keosd_client, subcmd).await?,
    }
