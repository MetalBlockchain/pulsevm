...
namespace pulsevm { namespace chain {
    ...
    *  In the context of pulsevm, every action is dispatched to the handler defined
   ...
   struct action : public action_base {
      ...
      action( vector<permission_level> auth, account_name account, action_name permission_name, const bytes& data )
            : action_base(account, permission_name, std::move(auth)), data(data)  {
      }
...
FC_REFLECT( pulsevm::chain::permission_level, (actor)(permission) )
FC_REFLECT( pulsevm::chain::action_base, (account)(name)(authorization) )
FC_REFLECT_DERIVED( pulsevm::chain::action, (pulsevm::chain::action_base), (data) )
