...
impl Database {
    ...
    pub fn create_account(&mut self, account_name: u64, creation_date: u32) -> Result<*const ffi::AccountObject, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn get_account(&self, account_name: u64) -> Result<&'static ffi::AccountObject, ChainError> {
        let guard = self.inner.read()?;
        let account = guard
            .find_account(account_name)
            .map_err(|e| ChainError::InternalError(format!("failed to get account: {}", e)))?;
        ...
        if account.is_null() {
            return Err(ChainError::InternalError(format!("account not found: {}", account_name)));
        }

        Ok(unsafe { &*account })
    }

    pub fn create_account_metadata(&mut self, account_name: u64, is_privileged: bool) -> Result<*const ffi::AccountMetadataObject, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn find_account_metadata(&self, account_name: u64) -> Result<*const ffi::AccountMetadataObject, ChainError> {
        let guard = self.inner.read()?;

        guard
            .find_account_metadata(account_name)
            .map_err(|e| ChainError::InternalError(format!("failed to find account metadata: {}", e)))
    }
    ...
    pub fn get_account_metadata(&self, account_name: u64) -> Result<&'static ffi::AccountMetadataObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
            .find_account_metadata(account_name)
            .map_err(|e| ChainError::InternalError(format!("failed to find account metadata: {}", e)))?;
    ...
    pub fn unlink_account_code(&mut self, old_code_entry: &ffi::CodeObject) -> Result<(), ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn create_undo_session(&mut self, enabled: bool) -> Result<cxx::UniquePtr<ffi::UndoSession>, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn initialize_account_resource_limits(&mut self, account_name: u64) -> Result<(), ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn add_transaction_usage(&mut self, accounts: &HashSet<u64>, cpu_usage: u64, net_usage: u64, time_slot: u32) -> Result<(), ChainError> {
    ...
    pub fn add_pending_ram_usage(&mut self, account_name: u64, ram_bytes: i64) -> Result<(), ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn set_account_limits(&mut self, account_name: u64, ram_bytes: i64, net_weight: i64, cpu_weight: i64) -> Result<bool, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn get_account_limits(&self, account_name: u64, ram_bytes: &mut i64, net_weight: &mut i64, cpu_weight: &mut i64) -> Result<(), ChainError> {
        let guard = self.inner.read()?;
    ...
    pub fn get_total_cpu_weight(&self) -> Result<u64, ChainError> {
        ...
        guard.get_total_cpu_weight().map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn get_total_net_weight(&self) -> Result<u64, ChainError> {
        let guard = self.inner.read()?;

        guard.get_total_net_weight().map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn get_account_net_limit(&self, name: u64, greylist_limit: u32) -> Result<ffi::NetLimitResult, ChainError> {
        let guard = self.inner.read()?;
    ...
    pub fn get_account_cpu_limit(&self, name: u64, greylist_limit: u32) -> Result<ffi::CpuLimitResult, ChainError> {
        let guard = self.inner.read()?;
    ...
    pub fn get_table(&self, code: u64, scope: u64, table: u64) -> Result<*const TableObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
            .find_table(code, scope, table)
            .map_err(|e| ChainError::InternalError(format!("{}", e)))?;
    ...
    pub fn create_table(&mut self, code: u64, scope: u64, table: u64, payer: u64) -> Result<*const TableObject, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
        let res = pinned
    ...
    pub fn db_find_i64(
        ...
        unsafe { pinned.db_find_i64(code, scope, table, id, keyval_cache.pin_mut()) }.map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn create_key_value_object(&mut self, table: &TableObject, payer: u64, id: u64, buffer: &[u8]) -> Result<*const KeyValueObject, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn update_key_value_object(&mut self, obj: &KeyValueObject, payer: u64, buffer: &[u8]) -> Result<(), ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn remove_table(&mut self, table: &TableObject) -> Result<(), ChainError> {
        ...
        pinned.remove_table(table).map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn is_account(&self, account: u64) -> Result<bool, ChainError> {
        let guard = self.inner.read()?;

        guard.is_account(account).map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn find_permission(&self, id: i64) -> Result<*const ffi::PermissionObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard.find_permission(id).map_err(|e| ChainError::InternalError(format!("{}", e)))?;

        Ok(res)
    }

    pub fn find_permission_by_actor_and_permission(&self, actor: u64, permission: u64) -> Result<*const ffi::PermissionObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
    ...
    pub fn get_permission_by_actor_and_permission(&self, actor: u64, permission: u64) -> Result<*const ffi::PermissionObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
            .find_permission_by_actor_and_permission(actor, permission)
            .map_err(|e| ChainError::InternalError(format!("{}", e)))?;
    ...
    pub fn link_auth(&mut self, account_name: u64, code_name: u64, requirement_name: u64, requirement_type: u64) -> Result<i64, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn get_code_object_by_hash(&self, code_hash: &CxxDigest, vm_type: u8, vm_version: u8) -> Result<*const ffi::CodeObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
    ...
    pub fn next_auth_sequence(&mut self, actor: u64) -> Result<u64, ChainError> {
        ...
        pinned.next_auth_sequence(actor).map_err(|e| ChainError::InternalError(format!("{}", e)))
    }
    ...
    pub fn next_global_sequence(&mut self) -> Result<u64, ChainError> {
        ...
        pinned.next_global_sequence().map_err(|e| ChainError::InternalError(format!("{}", e)))
    }

    pub fn db_remove_i64(&mut self, keyval_cache: &mut KeyValueIteratorCache, iterator: i32, receiver: u64) -> Result<i64, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn db_next_i64(&mut self, keyval_cache: &mut KeyValueIteratorCache, iterator: i32, primary: &mut u64) -> Result<i32, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn db_previous_i64(&mut self, keyval_cache: &mut KeyValueIteratorCache, iterator: i32, primary: &mut u64) -> Result<i32, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn db_end_i64(&mut self, keyval_cache: &mut KeyValueIteratorCache, code: u64, scope: u64, table: u64) -> Result<i32, ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn remove_permission(&mut self, permission: &ffi::PermissionObject) -> Result<(), ChainError> {
        let mut guard = self.inner.write()?;
        let pinned = guard.pin_mut();
    ...
    pub fn lookup_linked_permission(&self, account: u64, code: u64, requirement_type: u64) -> Result<Option<u64>, ChainError> {
        let guard = self.inner.read()?;
        let res = guard
    ...
    pub fn get_global_properties(&self) -> Result<*const ffi::GlobalPropertyObject, ChainError> {
        let guard = self.inner.read()?;
        let res = guard.get_global_properties().map_err(|e| ChainError::InternalError(format!("{}", e)))?;
