...
#include <pulsevm/chain/symbol.hpp>
...
namespace fc {
inline void to_variant(const pulsevm::chain::asset& var, fc::variant& vo) { vo = var.to_string(); }
inline void from_variant(const fc::variant& var, pulsevm::chain::asset& vo) {
   vo = pulsevm::chain::asset::from_string(var.get_string());
}
}

namespace fc {
inline void from_variant(const fc::variant& var, pulsevm::chain::extended_asset& vo) {
   if( var.is_array() ) {
...
FC_REFLECT(pulsevm::chain::asset, (amount)(sym))
FC_REFLECT(pulsevm::chain::extended_asset, (quantity)(contract) )
