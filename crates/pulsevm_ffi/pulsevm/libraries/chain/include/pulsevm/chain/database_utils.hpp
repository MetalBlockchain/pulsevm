...
namespace fc {
   ...
   inline
   void float128_to_uint128 (const float128_t& f, pulsevm::chain::uint128_t& u) {
      memcpy(&u, &f, sizeof(u));
   }

   inline
   void uint128_to_float128 (const pulsevm::chain::uint128_t& u,  float128_t& f) {
      memcpy(&f, &u, sizeof(f));
   }
   ...
   void to_variant( const float128_t& f, variant& v ) {
      ...
      char as_bytes[sizeof(pulsevm::chain::uint128_t)];
   ...
   void from_variant( const variant& v, float128_t& f ) {
      ...
      char temp[sizeof(pulsevm::chain::uint128_t)];
   ...
   inline
   void to_variant( const pulsevm::chain::shared_string& s, variant& v ) {
      v = variant(std::string(s.begin(), s.end()));
   }

   inline
   void from_variant( const variant& v, pulsevm::chain::shared_string& s ) {
      std::string _s;
      from_variant(v, _s);
      s = _s;
   }
   ...
   inline
   void to_variant( const pulsevm::chain::shared_blob& b, variant& v ) {
      v = variant(base64_encode(b.data(), b.size()));
   }

   inline
   void from_variant( const variant& v, pulsevm::chain::shared_blob& b ) {
      std::vector<char> b64 = base64_decode(v.as_string());
      b = std::string_view(b64.data(), b64.size());
   }

   template<typename T>
   void to_variant( const pulsevm::chain::shared_vector<T>& sv, variant& v ) {
      to_variant(std::vector<T>(sv.begin(), sv.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, pulsevm::chain::shared_vector<T>& sv ) {
      std::vector<T> _v;
      from_variant(v, _v);
      sv = v;
   }
   ...
   inline
   void to_variant(const pulsevm::chain::detail::snapshot_key_value_object& a, fc::variant& v) {
   ...
   inline
   void from_variant(const fc::variant& v, pulsevm::chain::detail::snapshot_key_value_object& a) {
...
DataStream& operator << ( DataStream& ds, const float128_t& v ) {
   pulsevm::chain::uint128_t uint128_v;
