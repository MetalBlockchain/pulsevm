...
namespace pulsevm { namespace chain {
   ...
   class table_id_object : public chainbase::object<table_id_object_type, table_id_object> {
      ...
      const name& get_code() const { return code; }
      const name& get_scope() const { return scope; }
      const name& get_table() const { return table; }
   ...
   struct key_value_object : public chainbase::object<key_value_object_type, key_value_object> {
      ...
      const name& get_payer() const { return payer; }
      const shared_blob& get_value() const { return value; }
   };
