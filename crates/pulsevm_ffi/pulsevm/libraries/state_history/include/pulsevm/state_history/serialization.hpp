...
#include <pulsevm/chain/trace.hpp>
...
datastream<ST>& history_serialize_container(datastream<ST>& ds, const std::vector<std::shared_ptr<T>>& v) {
   fc::raw::pack(ds, unsigned_int(v.size()));
   for (auto& x : v) {
      EOS_ASSERT(!!x, pulsevm::chain::plugin_exception, "null inside container");
      ds << make_history_serial_wrapper(*x);
   }
   return ds;
}
...
template <typename ST, typename T>
datastream<ST>& operator<<(datastream<ST>& ds, const pulsevm::state_history::big_vector_wrapper<T>& obj) {
...
datastream<ST>& operator<<(datastream<ST>& ds, const pulsevm::state_history::row_pair& rp) {
...
template <typename ST>
void history_pack_big_bytes(datastream<ST>& ds, const pulsevm::chain::bytes& v) {
...
template <typename ST>
void history_pack_big_bytes(datastream<ST>& ds, const pulsevm::chain::shared_blob& b) {
   fc::raw::pack(ds, unsigned_int((uint32_t)b.size()));
   ds.write(b.data(), b.size());
}

template <typename ST>
void history_pack_big_bytes(datastream<ST>& ds, const std::optional<pulsevm::chain::bytes>& v) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::account_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<pulsevm::chain::block_timestamp_type>(obj.obj.creation_date));
   fc::raw::pack(ds, as_type<pulsevm::chain::shared_string>(obj.obj.abi));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::account_metadata_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<bool>(obj.obj.is_privileged()));
   fc::raw::pack(ds, as_type<fc::time_point>(obj.obj.last_code_update));
   bool has_code = obj.obj.code_hash != pulsevm::chain::digest_type();
   fc::raw::pack(ds, has_code);
   if (has_code) {
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
      fc::raw::pack(ds, as_type<pulsevm::chain::digest_type>(obj.obj.code_hash));
   }
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::code_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
   fc::raw::pack(ds, as_type<pulsevm::chain::digest_type>(obj.obj.code_hash));
   fc::raw::pack(ds, as_type<pulsevm::chain::shared_string>(obj.obj.code));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::table_id_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<pulsevm::chain::table_id_object, pulsevm::chain::key_value_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
void serialize_secondary_index_data(datastream<ST>& ds, const pulsevm::chain::key256_t& obj) {
...
template <typename ST, typename T>
datastream<ST>& serialize_secondary_index(datastream<ST>& ds, const pulsevm::chain::table_id_object& context, const T& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<pulsevm::chain::table_id_object, pulsevm::chain::index64_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<pulsevm::chain::table_id_object, pulsevm::chain::index128_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<pulsevm::chain::table_id_object, pulsevm::chain::index256_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::chain_config>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::wasm_config>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::global_property_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(1));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::chain_config>(obj.obj.configuration)));
   fc::raw::pack(ds, as_type<pulsevm::chain::chain_id_type>(obj.obj.chain_id));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::wasm_config>(obj.obj.wasm_configuration)));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::protocol_state_object::activated_protocol_feature>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<pulsevm::chain::digest_type>(obj.obj.feature_digest));
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.activation_block_num));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::protocol_state_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::shared_key_weight>& obj) {
   fc::raw::pack(ds, as_type<pulsevm::chain::public_key_type>(obj.obj.key.to_public_key()));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::permission_level>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::permission_level_weight>& obj) {
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::permission_level>(obj.obj.permission)));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::wait_weight>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::shared_authority>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::permission_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.perm_name.to_uint64_t()));
   if (obj.obj.parent._id) {
      auto&       index  = obj.db.get_index<pulsevm::chain::permission_index>();
      ...
      if (!parent) {
         ...
         EOS_ASSERT(it != undo.removed_values.end(), pulsevm::chain::plugin_exception,
                    "can not find parent of permission_object");
         parent = &*it;
      }
      fc::raw::pack(ds, as_type<uint64_t>(parent->perm_name.to_uint64_t()));
   } else {
      fc::raw::pack(ds, as_type<uint64_t>(0));
   }
   fc::raw::pack(ds, as_type<fc::time_point>(obj.obj.last_updated));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<pulsevm::chain::shared_authority>(obj.obj.auth)));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::permission_link_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::resource_limits::resource_limits_object>& obj) {
   EOS_ASSERT(!obj.obj.pending, pulsevm::chain::plugin_exception,
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::resource_limits::usage_accumulator>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::resource_limits::resource_usage_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::usage_accumulator>(obj.obj.net_usage)));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::usage_accumulator>(obj.obj.cpu_usage)));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.ram_usage));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::resource_limits::resource_limits_state_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::usage_accumulator>(obj.obj.average_block_net_usage)));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::usage_accumulator>(obj.obj.average_block_cpu_usage)));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::resource_limits::ratio>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::resource_limits::elastic_limit_parameters>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   ...
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::ratio>(obj.obj.contract_rate)));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::resource_limits::ratio>(obj.obj.expand_rate)));
   return ds;
}

template <typename ST>
datastream<ST>&
operator<<(datastream<ST>& ds, const history_serial_wrapper<pulsevm::chain::resource_limits::resource_limits_config_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<pulsevm::chain::resource_limits::elastic_limit_parameters>(obj.obj.cpu_limit_parameters)));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<pulsevm::chain::resource_limits::elastic_limit_parameters>(obj.obj.net_limit_parameters)));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::action>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   history_serialize_container(ds, as_type<std::vector<pulsevm::chain::permission_level>>(obj.obj.authorization));
   fc::raw::pack(ds, as_type<pulsevm::chain::bytes>(obj.obj.data));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::action_receipt>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, as_type<pulsevm::chain::digest_type>(obj.obj.act_digest));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.global_sequence));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.recv_sequence));
   history_serialize_container(ds, as_type<flat_map<pulsevm::chain::name, uint64_t>>(obj.obj.auth_sequence));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper_stateless<pulsevm::chain::account_delta>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
...
inline std::optional<uint64_t> cap_error_code(const std::optional<uint64_t>& error_code) {
   ...
   const uint64_t upper_limit = static_cast<uint64_t>(pulsevm::chain::system_error_code::generic_system_error);
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<bool, pulsevm::chain::action_trace>& obj) {
   ...
   if (obj.obj.receipt) {
      fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::action_receipt>(*obj.obj.receipt)));
   }
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::action>(obj.obj.act)));
   ...
   history_serialize_container(ds, as_type<flat_set<pulsevm::chain::account_delta>>(obj.obj.account_ram_deltas));
   ...
   fc::raw::pack(ds, as_type<pulsevm::chain::bytes>(obj.obj.return_value));

   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<std::pair<uint8_t, bool>, pulsevm::state_history::augmented_transaction_trace>& obj) {
   auto& trace      = *obj.obj.trace;
   bool  debug_mode = obj.context.second;
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<pulsevm::chain::transaction_id_type>(trace.id));
   if (trace.receipt) {
      if (trace.failed_dtrx_trace && trace.receipt->status.value == pulsevm::chain::transaction_receipt_header::soft_fail)
         fc::raw::pack(ds, uint8_t(pulsevm::chain::transaction_receipt_header::executed));
      else
   ...
   history_context_serialize_container(ds, debug_mode, as_type<std::vector<pulsevm::chain::action_trace>>(trace.action_traces));

   fc::raw::pack(ds, bool(trace.account_ram_delta));
   if (trace.account_ram_delta) {
      fc::raw::pack(ds, make_history_serial_wrapper(as_type<pulsevm::chain::account_delta>(*trace.account_ram_delta)));
   }
   ...
   if (trace.failed_dtrx_trace) {
      uint8_t stat = pulsevm::chain::transaction_receipt_header::hard_fail;
      if (trace.receipt && trace.receipt->status.value == pulsevm::chain::transaction_receipt_header::soft_fail)
         stat = pulsevm::chain::transaction_receipt_header::soft_fail;
      std::pair<uint8_t, bool> context = std::make_pair(stat, debug_mode);
      fc::raw::pack(ds, make_history_context_wrapper(context, pulsevm::state_history::augmented_transaction_trace{trace.failed_dtrx_trace, obj.obj.partial}));
   }
   ...
   if (include_partial) {
      ...
      fc::raw::pack(ds, fc::unsigned_int(0));
      fc::raw::pack(ds, as_type<pulsevm::chain::time_point_sec>(partial.expiration));
      ...
      fc::raw::pack(ds, as_type<pulsevm::chain::extensions_type>(partial.transaction_extensions));
      fc::raw::pack(ds, as_type<std::vector<pulsevm::chain::signature_type>>(partial.signatures));
      fc::raw::pack(ds, as_type<std::vector<pulsevm::chain::bytes>>(partial.context_free_data));
   }
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper_stateless<bool, pulsevm::state_history::augmented_transaction_trace>& obj) {
   std::pair<uint8_t, bool> context = std::make_pair(pulsevm::chain::transaction_receipt_header::hard_fail, obj.context);
   ds << make_history_context_wrapper(context, obj.obj);
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const pulsevm::state_history::get_blocks_result_v0& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const pulsevm::state_history::get_blocks_result_base& obj) {
   fc::raw::pack(ds, obj.head);
   fc::raw::pack(ds, obj.last_irreversible);
   fc::raw::pack(ds, obj.this_block);
   fc::raw::pack(ds, obj.prev_block);
