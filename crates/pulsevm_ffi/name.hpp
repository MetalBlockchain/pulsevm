...
namespace fc {
  class variant;
  void to_variant(const pulsevm::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, pulsevm::chain::name& check);
} // fc
...
namespace pulsevm::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, pulsevm::chain::name& check);
...
namespace std {
   template<> struct hash<pulsevm::chain::name> : private hash<uint64_t> {
      typedef pulsevm::chain::name argument_type;
...
FC_REFLECT( pulsevm::chain::name, (value) )
