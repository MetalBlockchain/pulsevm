...
use super::{transaction, transaction_context, Action, Name, TransactionContext};
...
impl<'a, 'b> ApplyContext<'a, 'b> {
    pub fn new(
        ...
        let trace = transaction_context.get_action_trace(action_ordinal).unwrap();
    ...
    pub fn exec(&mut self) -> Result<(), ApplyContextError> {
        self.notified.push_back((self.receiver.clone(), self.action_ordinal));
        self.exec_one()?;
        ...
        for ordinal in self.inline_actions.iter() {
            self.transaction_context.execute_action(*ordinal, self.recurse_depth + 1);
        }
