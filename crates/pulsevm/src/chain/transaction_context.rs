use super::{apply_context::{self, ApplyContext, ApplyContextError}, Action, ActionTrace, Name, Transaction};
...
impl<'a> TransactionContext<'a> {
    pub fn new(transaction: &'a Transaction) -> Self {
        Self { transaction, action_traces: Vec::new() }
    }
    ...
    pub fn schedule_action(&mut self, action: &Action, receiver: &Name, creator_action_ordinal: u32) -> u32 {
        let new_action_ordinal = (self.action_traces.len() as u32) + 1;
        let action_trace = ActionTrace::new(new_action_ordinal, creator_action_ordinal, receiver.clone(), action.clone());
        self.action_traces.push(action_trace);
        new_action_ordinal
    }

    pub fn execute_action(&mut self, action_ordinal: u32, recurse_depth: u32) -> Result<(), ApplyContextError> {
        // Execute the action
